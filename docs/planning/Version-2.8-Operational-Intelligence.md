...
- Retained mission forecasts preserve complete hourly Open-Meteo evidence and add schema/store v2 derived operating periods. Schema/store v1 records are normalized in memory on read, remain readable across restart without a read-time rewrite, and are upgraded to v2 only by an explicit save.
...
- Validation evidence: focused provider/store/API/UI/review/readiness tests pass (8 files, 168 tests); TypeScript check passes; production build passes; full automated suite passes (94 files, 985 tests); `git diff --check` passes. All twelve 2.8-02 acceptance gates are demonstrated by the implementation and regression coverage.
