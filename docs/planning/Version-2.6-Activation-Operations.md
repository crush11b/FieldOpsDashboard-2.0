...
The temporal-coherence rule is deliberately based on elapsed observations rather than exact sentence cadence: a valid RMC pair requires a positive UTC delta no greater than 10 seconds, a positive monotonic receipt interval, and `abs(UTC delta - receipt interval) <= 0.5` seconds. This accepts ordinary serial buffering and scheduling jitter; for example, a 1.000-second RMC progression received 0.8546914 seconds apart differs by only 0.1453086 seconds. Replayed or repeated timestamps fail the positive-delta check, burst-delivered evidence fails the receipt-interval comparison, stale evidence fails the freshness age check, and materially inconsistent progression fails the 0.5-second bound. The current passive comparison is the latest observation; an explicit synchronization result is retained separately as the last synchronization attempt, so a later passive poll cannot rewrite or conceal the operation result. A confirmed synchronization whose trusted offset is already within two seconds is a no-op with zero `SetUtc` calls; only a larger trusted offset inside the five-minute safety limit can request one correction.
