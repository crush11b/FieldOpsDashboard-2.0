...
import { createDashboardConfigRouter, DashboardConfigStore, getDefaultDashboardConfigPath, resolveDashboardCatalog, resolveWsjtxConfiguration } from './server/dashboardConfig';
...
import { createEquipmentRouter } from './server/equipmentApi';
import { EquipmentStore, getDefaultEquipmentPaths } from './server/equipmentStore';
...
async function startServer() {
  ...
  app.use(createLauncherRouter(() => resolveDashboardCatalog(dashboardConfigStore), new NamedPipeTrayLauncherClient()));
  app.use(createAppDiscoveryRouter({ catalogResolver: () => resolveDashboardCatalog(dashboardConfigStore) }));
  ...
  const equipmentPaths = getDefaultEquipmentPaths();
  const equipmentStore = new EquipmentStore(equipmentPaths.inventory, equipmentPaths.loadouts);
  ...
  app.use(createEquipmentRouter(equipmentStore));
