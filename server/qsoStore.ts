...
export const QSO_STORE_VERSION = 2 as const;
export const QSO_PREVIOUS_STORE_VERSION = 1 as const;
...
export class QsoStore {
  ...
  load(): QsoStoreReadResult {
    ...
    if (!isRecord(parsed) || (parsed.storeVersion !== QSO_STORE_VERSION && parsed.storeVersion !== QSO_PREVIOUS_STORE_VERSION) || !Array.isArray(parsed.qsos)) return { status: 'invalid', qsos: [], diagnostics: [{ code: isRecord(parsed) && parsed.storeVersion !== QSO_STORE_VERSION && parsed.storeVersion !== QSO_PREVIOUS_STORE_VERSION ? 'unsupported_store_version' : 'corrupt', message: 'The QSO store wrapper is unsupported or malformed.' }] };
  ...
  create(input: CreateQsoInput): { readonly qso: Qso; readonly diagnostics: readonly QsoStoreDiagnostic[] } { const loaded = this.load(); const qso = createQso(input, this.options); if (loaded.qsos.some(item => item.qsoId === qso.qsoId)) throw new Error('The QSO ID already exists.'); this.write({ storeVersion: QSO_STORE_VERSION, qsos: order([qso, ...loaded.qsos]).slice(0, QSO_MAX_RETAINED_RECORDS) }); return { qso, diagnostics: loaded.diagnostics }; }
  save(qso: Qso): { readonly qso: Qso; readonly diagnostics: readonly QsoStoreDiagnostic[] } { const loaded = this.load(); const normalized = normalizeQso(qso); if (!normalized.valid || !normalized.qso) throw new Error('The QSO value is invalid.'); this.write({ storeVersion: QSO_STORE_VERSION, qsos: order([normalized.qso, ...loaded.qsos.filter(item => item.qsoId !== qso.qsoId)]) }); return { qso: normalized.qso, diagnostics: loaded.diagnostics }; }
  delete(qsoId: string): { readonly status: 'deleted' | 'notFound'; readonly qso?: Qso; readonly diagnostics: readonly QsoStoreDiagnostic[] } { const loaded = this.load(); const qso = loaded.qsos.find(item => item.qsoId === qsoId); if (!qso) return { status: 'notFound', diagnostics: loaded.diagnostics }; this.write({ storeVersion: QSO_STORE_VERSION, qsos: loaded.qsos.filter(item => item.qsoId !== qsoId) }); return { status: 'deleted', qso, diagnostics: loaded.diagnostics }; }
