...
export function createActivationRouter(options: ActivationApiOptions): Router {
  ...
  router.patch('/api/activations/:activationId/objective', (request, response) => {
    const objectiveSelection = request.body?.objectiveSelection;
    if (!(ACTIVATION_OBJECTIVE_SELECTIONS as readonly string[]).includes(objectiveSelection)) { response.status(400).json(error('invalid_objective_selection', `Objective selection must be one of: ${ACTIVATION_OBJECTIVE_SELECTIONS.join(', ')}.`)); return; }
    const operatingObjective = request.body?.operatingObjective;
    if (objectiveSelection === 'explicitly_absent' ? operatingObjective !== undefined : !validateOperatingObjective(operatingObjective)) { response.status(400).json(error('invalid_operating_objective', objectiveSelection === 'explicitly_absent' ? 'An explicitly absent objective cannot include objective data.' : 'A selected objective must be valid.')); return; }
    const existing = options.store.get(request.params.activationId);
    if (existing.status === 'notFound') { const unavailable = hasIoError(existing.diagnostics); response.status(unavailable ? 503 : 404).json(error(unavailable ? 'persistence_unavailable' : 'not_found', unavailable ? 'Activations are temporarily unavailable.' : 'The Activation was not found.', existing.diagnostics)); return; }
    if (existing.activation.status === 'completed') { response.status(409).json(error('objective_locked', 'Completed Activations cannot change their objective.', existing.diagnostics)); return; }
    const issues = validateObjectiveSelection(existing.activation.type, objectiveSelection as ActivationObjectiveSelection, operatingObjective as ActivationOperatingObjective | undefined);
