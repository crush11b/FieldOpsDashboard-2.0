...
export async function retrieveMissionForecast(brief: SmartDeployBriefV2, options: MissionForecastProviderOptions = {}): Promise<MissionForecastRetrieval> {
  ...
  try {
    ...
    const requiredArrays = [hourly.time, hourly.temperature_2m, hourly.precipitation_probability, hourly.wind_speed_10m, hourly.wind_direction_10m, hourly.weather_code];
    if (requiredArrays.some(array => array.length !== hourly.time.length) || (hourly.wind_gusts_10m !== undefined && (!Array.isArray(hourly.wind_gusts_10m) || hourly.wind_gusts_10m.length !== hourly.time.length))) return unusable();
