...
export function buildOperationsReadinessSummary(input: OperationsReadinessInput): OperationsReadinessSummary {
  ...
  const propagationStatus: ReadinessStatus = input.propagation.status === 'modeled' || input.propagation.status === 'observed-only' ? 'ready' : input.propagation.status === 'partial' ? 'attention' : input.propagation.status === 'stale' ? 'stale' : 'unavailable';
  ...
  const clockStatus: ReadinessStatus = clock?.status === 'synchronized' ? 'ready' : clock?.status === 'unavailable' ? 'unavailable' : !clock || clock.status === 'unknown' ? 'unknown' : 'attention';
  const clockMessage = clock?.message ?? (clockStatus === 'ready' ? 'Windows time is synchronized from fresh GNSS UTC evidence.' : clock?.status === 'degraded' ? 'Windows time is degraded; digital decoding should remain possible, but timing should be improved.' : clock?.status === 'not_synchronized' ? 'Windows time is not synchronized; digital decoding is not expected to work reliably.' : 'Clock synchronization cannot currently be verified.');
  add({ id: 'clock-synchronization', status: clockStatus, priority: clockStatus === 'ready' ? 'low' : 'medium', message: clockMessage, source: clock?.source ?? SOURCE.clockSyncUnverified, ...(clock?.observedAtUtc ? { observedAtUtc: clock.observedAtUtc } : {}), ...(clockStatus === 'unavailable' ? { freshness: 'unavailable' as const } : {}), limitation: clock ? 'Synchronization evidence reflects the last explicit operator action; it does not continuously steer Windows time.' : 'No Windows clock-synchronization telemetry is available; system time, GPS time, and checklist state are not evidence.', recommendedAction: clockStatus === 'ready' ? undefined : 'Acquire fresh GNSS UTC and explicitly synchronize Windows time before offline operation.' });
