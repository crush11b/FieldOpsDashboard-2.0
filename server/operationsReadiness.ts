...
export const READINESS_STATUSES = ['ready', 'attention', 'blocked', 'unknown', 'stale', 'unavailable'] as const;
...
export function buildOperationsReadinessSummary(input: OperationsReadinessInput): OperationsReadinessSummary {
  ...
  if (!brief) {
    add({ id: 'sota-plan-missing', status: 'blocked', priority: 'high', message: 'No retained SOTA SmartDeploy plan is available.', source: input.plan.sotaDataset.source, recommendedAction: 'Create or retain a SOTA SmartDeploy plan before field departure.' });
  } else {
  ...
  const runtimeStatus: ReadinessStatus = input.power.runtimeValid && input.power.runtimeSeconds !== null && input.power.runtimeSeconds >= 0 ? 'ready' : 'unknown';
  ...
  const severeAlert = alerts?.active.find(alert => alert.severity === 'Extreme' || alert.severity === 'Severe');
  add({ id: 'weather-alerts', status: severeAlert ? 'blocked' : alerts?.status === 'live' ? 'ready' : alerts?.status === 'stale' ? 'stale' : 'unavailable', priority: severeAlert ? 'high' : alerts?.status === 'live' ? 'low' : 'medium', message: severeAlert ? `Active ${severeAlert.severity.toLowerCase()} weather alert: ${severeAlert.title}.` : alerts?.status === 'live' ? 'No severe weather alert is present in the available alert set.' : alerts?.status === 'stale' ? 'Weather alerts are stale.' : 'Weather alerts are unavailable.', source: alerts?.source ?? SOURCE.evaluator, ...(alerts?.observedAtUtc ? { observedAtUtc: alerts.observedAtUtc } : {}), freshness: alerts?.status === 'live' ? 'fresh' : alerts?.status === 'stale' ? 'stale' : 'unavailable', recommendedAction: severeAlert ? 'Review the alert before operating.' : undefined });
  ...
  add({ id: 'local-nvis', status: 'unknown', priority: 'low', message: 'Local/NVIS evaluation is unsupported.', source: SOURCE.unsupported, limitation: 'No Local/NVIS prediction or recommendation is implemented.' });
