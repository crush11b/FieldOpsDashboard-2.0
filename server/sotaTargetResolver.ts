...
export class SotaActivationTargetResolver implements ActivationTargetResolver {
  constructor(private readonly dataset: LocalSotaSummitDataset) {}
  ...
  async resolve(request: ActivationTargetRequest): Promise<SotaTargetResolution> {
    ...
    if (this.dataset.state === 'UNAVAILABLE') return { status: 'unavailable', reference, error: 'SOTA summit data is unavailable offline.' };
    const summit = this.dataset.get(reference);
    if (!summit) return { status: 'unknown', reference };
    const resolvedAtUtc = this.dataset.metadata?.downloadedAtUtc;
    ...
    const status = this.dataset.state === 'STALE' ? 'stale' : 'cached';
