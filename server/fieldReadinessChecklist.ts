...
function isValidFieldReadinessChecklistId(input: unknown): input is string { return typeof input === 'string' && input.length > 0 && input.length <= FIELD_READINESS_CHECKLIST_MAX_ID_LENGTH && /^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(input); }
