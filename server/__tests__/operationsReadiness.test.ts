...
describe('buildOperationsReadinessSummary', () => {
  ...
  it('attributes a missing plan to the evaluator rather than the SOTA dataset', () => { const base = baseInput(); const result = buildOperationsReadinessSummary({ ...base, plan: { ...base.plan, brief: null } }); expect(result.findings.find(f => f.id === 'sota-plan-missing')?.source.id).toBe('operations-readiness-evaluator'); });
