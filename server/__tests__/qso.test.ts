...
describe('QSO model and persistence', () => {
  ...
  it('migrates singular references without changing canonical identity', () => { const legacy = normalizeQso({ ...input, schemaVersion: 1, qsoId: 'qso-legacy', potaRef: 'US-0182', createdAtUtc: '2026-08-25T12:00:00Z', updatedAtUtc: '2026-08-25T12:00:00Z' }); expect(legacy.qso?.schemaVersion).toBe(2); expect(legacy.qso?.entityAssociations.entities).toMatchObject([{ program: 'POTA', reference: 'US-0182', source: 'legacy_migration' }]); const fingerprint = qsoFingerprint(legacy.qso!); const edited = { ...legacy.qso!, entityAssociations: { schemaVersion: 1 as const, entities: [] } }; expect(qsoFingerprint(edited)).toBe(fingerprint); });
