...
const qso = (id: string, time: string, band: string, mode: string, source: 'manual' | 'wsjtx' | 'adif_import') => createQso({ activationId: 'activation-1', qsoId: id, qsoDateTimeUtc: time, callsign: id === 'qso-1' ? 'W1AW' : 'K1ABC', band, mode, source, createdAtUtc: time, updatedAtUtc: time } as any);
...
describe('Activation Review assembly', () => {
  it('summarizes QSO counts, chronology, callsigns, and provenance', () => {
    const review = assembleActivationReview(dependencies([qso('qso-1', '2026-08-25T12:00:00Z', '20m', 'SSB', 'manual'), qso('qso-2', '2026-08-25T12:05:00Z', '40m', 'FT8', 'wsjtx'), qso('qso-3', '2026-08-25T12:06:00Z', '15m', 'FT8', 'adif_import')]));
    expect(review.results).toMatchObject({ total: 3, byBand: { '20m': 1, '40m': 1, '15m': 1 }, byMode: { SSB: 1, FT8: 2 }, firstQsoUtc: '2026-08-25T12:00:00.000Z', lastQsoUtc: '2026-08-25T12:06:00.000Z', uniqueCallsigns: 2, manual: 1, wsjtx: 1, adifImported: 1 });
