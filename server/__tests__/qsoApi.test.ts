...
describe('QSO API', () => {
  it('supports manual CRUD, Activation scoping, import duplicate reporting, and export', async () => { const api = await setup(); try { const root = `${api.base}/api/activations/${api.activation.activationId}/qsos`; const created = await json(root, post({ qsoDateTimeUtc: '2026-08-25T11:00:00Z', callsign: 'w1aw', band: '20m', mode: 'SSB' })); expect(created.status).toBe(201); expect(created.body.qso.entityAssociations.entities).toMatchObject([{ program: 'POTA', reference: 'US-0182', source: 'active_operation' }]); const qsoId = created.body.qso.qsoId; const updated = await json(`${root}/${qsoId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ callsign: 'K1ABC', qsoDateTimeUtc: '2026-08-25T11:01:00Z', band: '20m', mode: 'SSB', entityAssociations: { schemaVersion: 1, entities: [] } }) }); expect(updated.body.qso.callsign).toBe('K1ABC'); expect(updated.body.qso.qsoId).toBe(qsoId); expect(updated.body.qso.entityAssociations.entities).toEqual([]); const adif = '<QSO_DATE:8>20260825<TIME_ON:6>120000<CALL:4>W1AW<BAND:3>20M<MODE:3>FT8<EOR>'; const imported = await json(`${root}/import`, post({ content: adif })); const repeated = await json(`${root}/import`, post({ content: adif })); expect(imported.body.imported).toBe(1); expect(repeated.body.duplicates).toBe(1); const exported = await fetch(`${root}/export`); expect(exported.status).toBe(200); expect(await exported.text()).toContain('<CALL:4>W1AW'); const deleted = await fetch(`${root}/${qsoId}`, { method: 'DELETE' }); expect(deleted.status).toBe(200); } finally { await new Promise<void>(resolve => api.server.close(() => resolve())); } });
