...
describe('V2.8-03 Station Signal Observation foundation', () => {
  it('replaces open TX Context, filters compatible outbound reports, and retains factual summaries', () => {
    let id = 0;
    const store = new OperationalIntelligenceStore(tempPath(), { now: () => NOW, createId: () => `id-${++id}`, operatorCallsign: () => 'K1ABC' });
    const first = store.openTxContext('activation-1', contextInput()).context;
    const second = store.openTxContext('activation-1', contextInput()).context;
    expect(first.endedAtUtc).toBeUndefined();
    expect(store.list('activation-1').txContexts.find(item => item.segmentId === first.segmentId)?.endedAtUtc).toBe(NOW.toISOString());
    const result = store.captureObservation('activation-1', second.segmentId, snapshot([report('1', 'K1ABC', 'FT8', NOW.toISOString()), report('2', 'K1ABC', 'SSB', NOW.toISOString()), report('3', 'OTHER', 'FT8', NOW.toISOString())]));
  ...
  it('preserves exact zero-report semantics and closes open context', () => {
    ...
    const store = new OperationalIntelligenceStore(filePath, { now: () => NOW, createId: () => 'segment-1', operatorCallsign: () => 'K1ABC' });
    const context = store.openTxContext('activation-1', contextInput()).context;
    const observation = store.captureObservation('activation-1', context.segmentId, snapshot([], 'unavailable')).observation;
    expect(observation).toMatchObject({ matchingReportCount: 0, uniqueReceiverCount: 0, newestMatchingReportAtUtc: null, limitations: ['Observed digital reception does not prove RF transmission, SSB usability, station success, or contact probability.', 'No matching reports observed'] });
    store.closeActivation('activation-1');
  ...
  it('serves the Activation-scoped API from the same store and observed service', async () => {
    ...
    activationStore.create({ type: 'General' });
