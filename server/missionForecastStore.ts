...
interface StoreDocument { readonly storeVersion: typeof MISSION_FORECAST_STORE_VERSION; readonly records: readonly MissionForecastRecord[]; }
...
export class MissionForecastStore {
  ...
  load(): MissionForecastStoreReadResult {
    ...
    for (const candidate of parsed.records) { if (!validateMissionForecastRecord(candidate)) diagnostics.push({ code: 'invalid_record', message: 'A stored mission forecast was skipped because required fields were invalid.', briefId: isRecord(candidate) && typeof candidate.briefId === 'string' ? candidate.briefId : undefined }); else if (records.some(record => record.briefId === candidate.briefId)) diagnostics.push({ code: 'invalid_record', message: 'A duplicate stored mission forecast was skipped.', briefId: candidate.briefId }); else records.push(candidate); }
