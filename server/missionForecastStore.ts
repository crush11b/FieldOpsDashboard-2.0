...
import { normalizeRetainedMissionForecast, validateMissionForecastRecord, type MissionForecastRecord } from './missionForecast';

export const MISSION_FORECAST_STORE_VERSION = 2 as const;
export const MISSION_FORECAST_PREVIOUS_STORE_VERSION = 1 as const;
...
export class MissionForecastStore {
  ...
  load(): MissionForecastStoreReadResult {
    ...
    if (!isRecord(parsed) || (parsed.storeVersion !== MISSION_FORECAST_STORE_VERSION && parsed.storeVersion !== MISSION_FORECAST_PREVIOUS_STORE_VERSION) || !Array.isArray(parsed.records)) return { status: 'invalid', records: [], diagnostics: [{ code: isRecord(parsed) && parsed.storeVersion !== MISSION_FORECAST_STORE_VERSION && parsed.storeVersion !== MISSION_FORECAST_PREVIOUS_STORE_VERSION ? 'unsupported_store_version' : 'corrupt', message: 'The mission forecast store wrapper is unsupported or malformed.' }] };
    const records: MissionForecastRecord[] = []; const diagnostics: MissionForecastStoreDiagnostic[] = [];
    for (const candidate of parsed.records) { const normalized = isRecord(candidate) && typeof candidate.briefId === 'string' ? normalizeRetainedMissionForecast(candidate.record) : null; if (!isRecord(candidate) || typeof candidate.briefId !== 'string' || !normalized || normalized.briefId !== candidate.briefId) diagnostics.push({ code: 'invalid_record', message: 'A stored mission forecast was skipped because its key and brief identity were invalid.', briefId: isRecord(candidate) && typeof candidate.briefId === 'string' ? candidate.briefId : undefined }); else if (records.some(record => record.briefId === candidate.briefId)) diagnostics.push({ code: 'invalid_record', message: 'A duplicate stored mission forecast was skipped.', briefId: candidate.briefId }); else records.push(markRetained(normalized)); }
  ...
  save(record: MissionForecastRecord): { readonly record: MissionForecastRecord; readonly diagnostics: readonly MissionForecastStoreDiagnostic[] } { const normalized = normalizeRetainedMissionForecast(record); if (!normalized) throw new Error('The mission forecast record is invalid.'); const loaded = this.load(); this.write({ storeVersion: MISSION_FORECAST_STORE_VERSION, records: [normalized, ...loaded.records.filter(candidate => candidate.briefId !== normalized.briefId)].map(candidate => ({ briefId: candidate.briefId, record: candidate })).sort((left, right) => right.record.updatedAtUtc.localeCompare(left.record.updatedAtUtc)).slice(0, MAX_RETAINED_RECORDS) }); return { record: normalized, diagnostics: loaded.diagnostics }; }
...
function markRetained(record: MissionForecastRecord): MissionForecastRecord { return { ...record, freshness: 'retained', operatingPeriods: record.operatingPeriods.map(period => ({ ...period, freshness: 'retained' })) }; }