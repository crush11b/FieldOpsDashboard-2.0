...
if (!parentPort) throw new Error('P.533 worker requires a parent port.');

parentPort.on('message', (message: WorkerRequest) => {
  const execution = executionQueue.then(() => executeSerialized(message.request));
  executionQueue = execution.then(() => undefined, () => undefined);
  void execution.then(
    result => parentPort?.postMessage({ id: message.id, result }),
    error => parentPort?.postMessage({ id: message.id, result: failure('engine_initialization_failed', errorMessage(error)) }),
  );
});
