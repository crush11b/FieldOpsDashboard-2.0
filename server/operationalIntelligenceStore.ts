...
function replaceFile(temporaryPath: string, targetPath: string): void { try { fs.renameSync(temporaryPath, targetPath); return; } catch (error) { if (!isNodeError(error) || (error.code !== 'EEXIST' && error.code !== 'EPERM')) throw error; } const backupPath = `${targetPath}.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.bak`; fs.renameSync(targetPath, backupPath); try { fs.renameSync(temporaryPath, targetPath); } catch (replacementError) { try { fs.renameSync(backupPath, targetPath); } catch (restoreError) { throw new Error(`Replacement failed and the previous store could not be restored: ${errorMessage(replacementError)}; backup retained at ${backupPath}.`, { cause: restoreError }); } throw replacementError; } try { fs.rmSync(backupPath, { force: true }); } catch {} }
