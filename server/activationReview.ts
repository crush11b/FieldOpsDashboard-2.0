...
export interface ActivationReview {
  ...
  readonly results: { readonly state: ReviewEvidenceState; readonly total: number; readonly byBand: Readonly<Record<string, number>>; readonly byMode: Readonly<Record<string, number>>; readonly firstQsoUtc: string | null; readonly lastQsoUtc: string | null; readonly uniqueCallsigns: number; readonly manual: number; readonly adifImported: number; readonly qsos: readonly Qso[] };
...
export function assembleActivationReview(dependencies: ActivationReviewDependencies): ActivationReview {
  ...
  if (!forecastResult || forecastResult.status !== 'found') findings.push('No retained mission forecast is available.');
  if (!spaceWeatherResult || spaceWeatherResult.status !== 'found') findings.push('No retained space-weather evidence is available.');
  if (!noteResult || noteResult.status !== 'found' && !('collections' in noteResult)) findings.push('No Activation Notes are present.');
  ...
  return {
    ...
    results: { state: qsoResult.diagnostics.some(item => item.code === 'io_error') ? 'error' : qsos.length ? 'available' : 'unknown', total: qsos.length, byBand: countBy(qsos, qso => qso.band), byMode: countBy(qsos, qso => qso.mode), firstQsoUtc: qsos[0]?.qsoDateTimeUtc ?? null, lastQsoUtc: qsos.at(-1)?.qsoDateTimeUtc ?? null, uniqueCallsigns: new Set(qsos.map(qso => qso.callsign)).size, manual: qsos.filter(qso => qso.source === 'manual').length, adifImported: qsos.filter(qso => qso.source === 'adif_import').length, qsos },
...
function bandsFromBrief(brief: SmartDeployBriefV2): string[] { return [...new Set(brief.sections.propagation.evidence.samples.flatMap(sample => sample.bands.map(band => band.band)))]; }
