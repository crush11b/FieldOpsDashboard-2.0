...
describe('centralized product metadata', () => {
  it('exposes the canonical release identity and valid semantic version', () => {
    ...
    expect(PRODUCT_METADATA.version).toBe('2.9.0');
    expect(PRODUCT_METADATA.version).toMatch(/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/);
    expect(PRODUCT_METADATA.releaseName).toBe('Field Product Completion');
  });
  ...
  it('derives user-agent, ZIP, ADIF, and sanitized diagnostic metadata', () => {
    expect(getProductUserAgent()).toBe('FieldOpsDashboard/2.9.0');
    expect(getProductUserAgent('NOAA/NWS')).toBe('FieldOpsDashboard/2.9.0 (NOAA/NWS)');
    expect(getVersionedDownloadFilename()).toBe('FieldOpsDashboard-2.9.0.zip');
    expect(PRODUCT_METADATA.adifProgramId).toBe('FieldOpsDashboard');
    expect(PRODUCT_METADATA.adifProgramVersion).toBe('2.9.0');
    expect(getDiagnosticProductMetadata()).toEqual({
      product: 'FieldOps Dashboard',
      version: '2.9.0',
      release: 'Field Product Completion',
    });
  });
