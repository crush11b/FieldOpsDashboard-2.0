...
export const MySignalPanel: React.FC<Props> = ({ activation, stationState = null, readOnly = false, plannedSetup }) => {
  ...
  return <section className="rounded-xl border border-violet-700/70 bg-violet-950/20 p-3 space-y-3" aria-label="MY SIGNAL">
    ...
    {!loading && !readOnly && activation.status === 'active' && (!openContext || replacingContext) && <form onSubmit={saveContext} className="grid grid-cols-2 gap-2 sm:grid-cols-3">
      ...
      <Field label="BAND"><select aria-label="MY SIGNAL BAND" value={form.band} onChange={event => editForm({ band: event.target.value })} className={inputClass}>{AMATEUR_BAND_OPTIONS.filter(option => (PROPAGATION_GUIDANCE_BANDS as readonly string[]).includes(option.value)).map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field>
      ...
      <Field label="FREQUENCY MHz"><input min="0.1" step="0.0001" type="number" aria-label="MY SIGNAL FREQUENCY MHz" value={form.frequencyMHz} onChange={event => editForm({ frequencyMHz: event.target.value })} className={inputClass} /></Field>
