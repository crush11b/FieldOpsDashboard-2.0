...
export const MissionContextStrip: React.FC<{ brief: SmartDeployBriefV2; activation: Activation | null; phase: 'plan' | 'prepare' | 'operate' | 'review'; qsoCount: number | null; onOpenOperate?: () => void }> = ({ brief, activation, phase, qsoCount, onOpenOperate }) => {
  ...
  return <section aria-label="Mission and operation context" className="space-y-2">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-[9px] font-black uppercase text-cyan-300">{phase} · operation context</p><h3 className="text-sm font-black uppercase text-slate-100">{context.identity.reference}{context.identity.displayName ? ` · ${context.identity.displayName}` : ''}</h3><p className="text-[10px] text-slate-400">{context.identity.program} · Activation {readableStatus(context.lifecycle.status)}</p></div>{activation && <button type="button" className="text-[11px] font-black text-amber-200 underline" onClick={onOpenOperate}>{qsoCount === null ? 'QSOs in OPERATE' : `${qsoCount} QSOs`}</button>}</div>
