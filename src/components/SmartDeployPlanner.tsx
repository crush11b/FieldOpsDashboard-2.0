...
import { MODE_OPTIONS, ANTENNA_OPTIONS, getDefaultHeightCategoryForDeployment, getDeploymentOptionsForAntenna, getHeightOptionsForDeployment } from '../propagation/stationProfileCatalog';
import { isHeightCategoryValidForDeployment, type DeploymentGeometry, type StationProfile } from '../propagation/domain';
...
import type { LoadoutRecord } from '../equipment/domain';
...
export const SmartDeployPlanner: React.FC<SmartDeployPlannerProps> = ({ operatingLocation, stationProfile }) => {
  ...
  const loadLoadouts = async () => {
    try { const response = await fetch('/api/loadouts'); if (!response.ok) throw new Error(); const payload = await response.json(); setLoadouts((payload.loadouts || []).filter((item: LoadoutRecord) => item.state === 'active')); } catch { setLoadouts([]); }
  };
  ...
  useEffect(() => { void Promise.all([loadRecent(), loadSotaDataStatus(), loadLoadouts()]); }, []);
  ...
  const generate = async () => {
    ...
    if (!activationReference.trim() || !start || !end || !radio.trim() || modes.length === 0 || !propagationRegion) { setError(`Complete the ${activationProgram} reference, mission window, required Radio, mode, and RF target region. Radio is required because the planning brief records the station equipment used for propagation context.`); return; }
  ...
  const setDeployment = (geometry: DeploymentGeometry) => {
    setDeploymentGeometry(geometry);
    setHeightCategory(current => isHeightCategoryValidForDeployment(geometry, current)
      ? current
  ...
  return <div id="smartdeploy-planner" className="space-y-4">
    <div className="no-print p-3 rounded-xl border border-cyan-700/60 bg-cyan-950/20 flex flex-wrap items-center justify-between gap-3">
    ...
    <div className="no-print p-4 rounded-xl border border-amber-600/70 bg-amber-950/20 space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="text-[10px] uppercase text-slate-400 sm:col-span-2">Reusable loadout (optional)<select aria-label="Reusable loadout" value={loadoutId} onChange={event => setLoadoutId(event.target.value)} className="mt-1 w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded text-cyan-200 font-bold"><option value="">No retained loadout</option>{loadouts.map(item => <option key={item.loadoutId} value={item.loadoutId}>{item.name}</option>)}</select><span className="block mt-1 normal-case text-slate-500">The selected loadout and its current equipment facts will be frozen into this plan and Activation. Later inventory edits will not rewrite it.</span></label>
        <label className="text-[10px] uppercase text-slate-400">Radio <span className="text-amber-300">(required)</span><input aria-label="Radio" required value={radio} onChange={event => setRadio(event.target.value)} placeholder="Radio model or name" className="mt-1 w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded text-amber-200 font-bold" /><span className="block mt-1 normal-case text-slate-500">Required because the planning brief records the station equipment used for propagation context.</span></label>
      ...
      <button type="button" onClick={generate} disabled={busy} className="fo-control-primary w-full rounded border px-4 py-3 text-xs font-black shadow-lg transition duration-150 hover:shadow-xl active:translate-y-px active:shadow-sm disabled:cursor-not-allowed disabled:opacity-50">{busy ? 'GENERATING SMARTDEPLOY PLAN...' : 'GENERATE SMARTDEPLOY PLAN'}</button>
