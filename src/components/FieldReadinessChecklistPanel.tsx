...
interface FieldReadinessChecklistPanelProps { readonly brief: SmartDeployBrief; readonly readOnly?: boolean; }
type LoadState = 'loading' | 'not_started' | 'ready' | 'error';

export const FieldReadinessChecklistPanel: React.FC<FieldReadinessChecklistPanelProps> = ({ brief, readOnly = false }) => {
  ...
  return <section id="field-readiness-checklist" aria-label="Field Readiness Checklist" className="rounded-xl border border-emerald-700/60 bg-emerald-950/20 p-3 space-y-3">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      {checklist && <div className="flex flex-wrap items-center gap-2"><span className="text-[10px] text-slate-400">Updated <time dateTime={checklist.updatedAtUtc}>{formatUtc(checklist.updatedAtUtc)}</time></span>{!readOnly && !confirmReset && <button type="button" onClick={() => setConfirmReset(true)} disabled={saving} className="px-3 py-2 rounded border border-amber-700 text-amber-200 text-[10px] font-bold disabled:opacity-50">RESET CHECKLIST</button>}</div>}
    </div>
    ...
    {loadState === 'not_started' && <div className="space-y-2"><p className="text-[11px] text-slate-300">No checklist has been started for this SmartDeploy brief.</p>{!readOnly && <button type="button" onClick={() => void start()} disabled={saving} className="w-full py-3 rounded bg-emerald-500 text-slate-950 text-xs font-black disabled:opacity-50">{saving ? 'STARTING CHECKLIST...' : 'START CHECKLIST'}</button>}{message && <p role="alert" className="text-[11px] text-red-200">{message}</p>}</div>}
