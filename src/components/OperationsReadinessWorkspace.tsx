...
const ReadinessContent: React.FC<{
  ...
  const [objectiveSelection, setObjectiveSelection] = useState<ActivationObjectiveSelection>(initialActivation?.objectiveSelection || (initialActivation?.operatingObjective ? 'operator_entered' : brief.activation.program === 'POTA' || brief.activation.program === 'SOTA' ? 'program_default' : 'explicitly_absent'));
  useEffect(() => { if (initialActivation?.operatingObjective) setObjectiveDraft(draftFromObjective(initialActivation.operatingObjective)); setObjectiveSelection(initialActivation?.objectiveSelection || (initialActivation?.operatingObjective ? 'operator_entered' : brief.activation.program === 'POTA' || brief.activation.program === 'SOTA' ? 'program_default' : 'explicitly_absent')); }, [initialActivation, brief.activation.program]);
  const [objectiveError, setObjectiveError] = useState<string | null>(null);
  const [objectiveUtc, setObjectiveUtc] = useState<string | null>(null);
  ...
  const save = () => { if (objectiveSelection === 'explicitly_absent') { void (onSaveObjective || onStartActivation)?.(undefined, objectiveSelection); return; } const normalized = normalizeActivationObjective(objectiveDraft); if (!normalized.objective) { setObjectiveError(normalized.error || 'The objective is invalid.'); return; } setObjectiveUtc(normalized.deadlineUtc ?? null); void (onSaveObjective || onStartActivation)?.(normalized.objective, objectiveSelection); };
  return <>
    <section className="rounded-lg border border-emerald-700/70 bg-emerald-950/20 p-3 space-y-2" aria-label="Summit readiness summary">
      ...
      {(onStartActivation || onSaveObjective) && <section className="rounded border border-emerald-800 bg-slate-950/40 p-2 space-y-2" aria-label="Activation objective"><h4 className="text-[10px] font-black uppercase text-emerald-300">ACTIVATION OBJECTIVE</h4><p className="text-[11px] text-slate-200">Choose an explicit objective for this Activation, or record that no objective is selected.</p><div className="flex flex-wrap gap-2"><button type="button" onClick={chooseDefault} className={`min-h-10 rounded border px-3 py-2 text-[10px] font-bold ${objectiveSelection === 'program_default' ? 'border-emerald-400 text-emerald-200' : 'border-slate-700 text-slate-300'}`}>ACCEPT PROPOSED DEFAULT</button><button type="button" onClick={() => { setObjectiveSelection('operator_entered'); setObjectiveError(null); }} className={`min-h-10 rounded border px-3 py-2 text-[10px] font-bold ${objectiveSelection === 'operator_entered' ? 'border-emerald-400 text-emerald-200' : 'border-slate-700 text-slate-300'}`}>CUSTOMIZE</button><button type="button" onClick={withoutObjective} className={`min-h-10 rounded border px-3 py-2 text-[10px] font-bold ${objectiveSelection === 'explicitly_absent' ? 'border-amber-400 text-amber-200' : 'border-slate-700 text-slate-300'}`}>PROCEED WITHOUT OBJECTIVE</button></div>{objectiveSelection !== 'explicitly_absent' && <div className="grid grid-cols-1 gap-2 sm:grid-cols-4"><label className="text-[9px] font-bold uppercase text-slate-400">Goal<select aria-label="Activation objective goal" value={objectiveDraft.goal} onChange={event => updateObjective({ goal: event.target.value as ActivationObjectiveDraft['goal'] })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100"><option value="secure_activation">Secure activation</option><option value="maximize_contacts">Maximize contacts</option><option value="chase_dx">Chase DX</option><option value="explore_bands">Explore bands</option></select></label><label className="text-[9px] font-bold uppercase text-slate-400">Label<input aria-label="Activation objective label" value={objectiveDraft.label} onChange={event => updateObjective({ label: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Threshold (QSOs)<input aria-label="Activation objective threshold" type="number" min="1" step="1" value={objectiveDraft.requiredQsoCount} onChange={event => updateObjective({ requiredQsoCount: event.target.value, thresholdProvenance: 'operator_entered' })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Deadline local<input aria-label="Activation objective deadline local" type="datetime-local" value={objectiveDraft.deadlineLocal} onChange={event => updateObjective({ deadlineLocal: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" />{objectiveDraft.deadlineLocal ? <span className="block mt-1 normal-case text-slate-500">Local ToughBook time; UTC {objectiveUtc || 'derived when validated'}</span> : <span className="block mt-1 normal-case text-slate-500">No explicit deadline selected</span>}</label></div>}<p className="text-[10px] text-slate-400">Selection provenance: {objectiveSelection === 'program_default' ? 'Accepted proposed program default.' : objectiveSelection === 'operator_entered' ? 'Operator entered or customized.' : 'Explicitly absent.'}</p>{objectiveError && <p role="alert" className="text-[10px] text-red-200">{objectiveError}</p>}<button type="button" disabled={starting} onClick={save} className="min-h-11 rounded border border-emerald-600 px-4 py-2 text-[11px] font-black text-emerald-200 disabled:opacity-50">{starting ? 'SAVING...' : initialActivation ? 'SAVE OBJECTIVE' : 'START ACTIVATION'}</button></section>}
