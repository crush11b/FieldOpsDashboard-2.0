...
const ReadinessContent: React.FC<{
  ...
  return <>
    <section className="rounded-lg border border-emerald-700/70 bg-emerald-950/20 p-3 space-y-2" aria-label="Summit readiness summary">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{readinessCard('Location', `${brief.plannedOperatingSite.location.gridSquare || 'Site unavailable'} / ${findingMessage(summary, 'current-location')}`, findFinding(summary, 'current-location')?.status)}{readinessCard('GPS', findingMessage(summary, 'current-location'), findFinding(summary, 'current-location')?.status)}<ClockReadinessCard finding={findFinding(summary, 'clock-synchronization')} onSynchronizeClock={onSynchronizeClock} />{readinessCard('ToughBook', summary.toughBook.status === 'ready' ? `${summary.toughBook.chargePercent ?? 'Unknown'}% / ${summary.toughBook.powerSource}` : 'Power unknown', summary.toughBook.status)}{readinessCard('Weather', weatherText, displayEvidence.weather.status === 'not_requested' ? 'unknown' : findFinding(summary, 'weather')?.status)}{readinessCard('Alerts', alertsText, displayEvidence.alerts.status === 'not_requested' ? 'unknown' : findFinding(summary, 'weather-alerts')?.status)}{readinessCard('Space Weather', 'Retained plan evidence', 'ready')}{readinessCard('Propagation', findingMessage(summary, 'propagation-evidence'), findFinding(summary, 'propagation-evidence')?.status)}{readinessCard('Checklist', findingMessage(summary, 'field-readiness-checklist'), findFinding(summary, 'field-readiness-checklist')?.status, findFinding(summary, 'field-readiness-checklist')?.message.includes('has not been started') ? 'NOT STARTED' : undefined)}</div>
      {onStartActivation && <section className="rounded border border-emerald-800 bg-slate-950/40 p-2 space-y-2" aria-label="Activation objective"><h4 className="text-[10px] font-black uppercase text-emerald-300">ACTIVATION OBJECTIVE</h4><div className="grid grid-cols-1 gap-2 sm:grid-cols-4"><label className="text-[9px] font-bold uppercase text-slate-400">Goal<select aria-label="Activation objective goal" value={objectiveDraft.goal} onChange={event => updateObjective({ goal: event.target.value as ActivationObjectiveDraft['goal'] })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100"><option value="secure_activation">Secure activation</option><option value="maximize_contacts">Maximize contacts</option><option value="chase_dx">Chase DX</option><option value="explore_bands">Explore bands</option></select></label><label className="text-[9px] font-bold uppercase text-slate-400">Label<input aria-label="Activation objective label" value={objectiveDraft.label} onChange={event => updateObjective({ label: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Threshold (QSOs)<input aria-label="Activation objective threshold" type="number" min="1" step="1" value={objectiveDraft.requiredQsoCount} onChange={event => updateObjective({ requiredQsoCount: event.target.value, thresholdProvenance: 'operator_entered' })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Deadline local<input aria-label="Activation objective deadline local" type="datetime-local" value={objectiveDraft.deadlineLocal} onChange={event => updateObjective({ deadlineLocal: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" />{objectiveDraft.deadlineLocal && <span className="block mt-1 normal-case text-slate-500">Local ToughBook time{objectiveUtc ? `; UTC ${objectiveUtc}` : '; UTC derived when validated'}</span>}</label></div><label className="flex items-center gap-2 text-[10px] text-slate-300"><input aria-label="Operator entered threshold provenance" type="checkbox" checked={objectiveDraft.thresholdProvenance === 'operator_entered'} onChange={event => updateObjective({ thresholdProvenance: event.target.checked ? 'operator_entered' : 'program_default' })} /> Threshold was entered or confirmed by operator</label>{objectiveError && <p role="alert" className="text-[10px] text-red-200">{objectiveError}</p>}<button type="button" disabled={starting} onClick={start} className="min-h-11 rounded border border-emerald-600 px-4 py-2 text-[11px] font-black text-emerald-200 disabled:opacity-50">{starting ? 'STARTING ACTIVATION...' : 'START ACTIVATION'}</button></section>}
...
const ClockReadinessCard: React.FC<{ finding?: ReadinessFinding; onSynchronizeClock: () => void }> = ({ finding, onSynchronizeClock }) => { const [confirmed, setConfirmed] = useState(false); const status = finding?.status; const canSynchronize = status === 'attention' || status === 'ready'; return <div className="rounded border border-slate-800 bg-slate-900/70 p-2"><span className="block text-[9px] uppercase text-slate-500">Clock</span><strong className="block mt-0.5 text-[10px] text-slate-200">{status ? status.toUpperCase() : 'UNKNOWN'}</strong><span className="block mt-0.5 text-[10px] text-slate-400 break-words">{finding?.message || 'Unknown'}</span>{canSynchronize && <div className="mt-2 space-y-1"><label className="flex items-start gap-1 text-[9px] text-amber-100"><input aria-label="Confirm Windows clock synchronization" type="checkbox" className="mt-0.5" checked={confirmed} onChange={event => setConfirmed(event.currentTarget.checked)} /> Confirm fresh GNSS UTC may set Windows time.</label><button type="button" disabled={!confirmed} className="min-h-8 px-2 py-1 rounded border border-cyan-700 text-cyan-200 text-[9px] font-bold disabled:opacity-50" onClick={() => { onSynchronizeClock(); setConfirmed(false); }}>SYNCHRONIZE WINDOWS TIME</button></div>}</div>; };
