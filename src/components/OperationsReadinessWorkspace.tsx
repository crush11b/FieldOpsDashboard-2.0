...
import type { ActivationOperatingObjective } from '../../server/activation';
import type { ClockSynchronizationEvidence } from '../../server/locationTelemetryPipe';
import { defaultActivationObjective, normalizeActivationObjective, type ActivationObjectiveDraft } from '../operations/activationObjective';

interface OperationsReadinessWorkspaceProps { readonly brief: SmartDeployBriefV2; readonly onStartActivation?: (objective: ActivationOperatingObjective) => Promise<void>; }
...
const ReadinessContent: React.FC<{
  ...
  onStartActivation?: (objective: ActivationOperatingObjective) => Promise<void>;
  starting: boolean;
}> = ({ brief, summary, displayEvidence, liveLoading, message, clockAttempt, onLoadLiveWeather, onSynchronizeClock, onStartActivation, starting }) => {
  ...
  const [objectiveDraft, setObjectiveDraft] = useState<ActivationObjectiveDraft>(() => defaultActivationObjective(brief.activation.program));
  const [objectiveError, setObjectiveError] = useState<string | null>(null);
  const [objectiveUtc, setObjectiveUtc] = useState<string | null>(null);
  const updateObjective = (changes: Partial<ActivationObjectiveDraft>) => { setObjectiveDraft(current => ({ ...current, ...changes })); setObjectiveError(null); if ('deadlineLocal' in changes) setObjectiveUtc(null); };
  const start = () => { const normalized = normalizeActivationObjective(objectiveDraft); if (!normalized.objective) { setObjectiveError(normalized.error || 'The objective is invalid.'); return; } setObjectiveUtc(normalized.deadlineUtc ?? null); void onStartActivation?.(normalized.objective); };
  return <>
    <section className="rounded-lg border border-emerald-700/70 bg-emerald-950/20 p-3 space-y-2" aria-label="Summit readiness summary">
      ...
      {onStartActivation && <section className="rounded border border-emerald-800 bg-slate-950/40 p-2 space-y-2" aria-label="Activation objective"><h4 className="text-[10px] font-black uppercase text-emerald-300">ACTIVATION OBJECTIVE</h4><div className="grid grid-cols-1 gap-2 sm:grid-cols-4"><label className="text-[9px] font-bold uppercase text-slate-400">Goal<select aria-label="Activation objective goal" value={objectiveDraft.goal} onChange={event => updateObjective({ goal: event.target.value as ActivationObjectiveDraft['goal'] })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100"><option value="secure_activation">Secure activation</option><option value="maximize_contacts">Maximize contacts</option><option value="chase_dx">Chase DX</option><option value="explore_bands">Explore bands</option></select></label><label className="text-[9px] font-bold uppercase text-slate-400">Label<input aria-label="Activation objective label" value={objectiveDraft.label} onChange={event => updateObjective({ label: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Threshold (QSOs)<input aria-label="Activation objective threshold" type="number" min="1" step="1" value={objectiveDraft.requiredQsoCount} onChange={event => updateObjective({ requiredQsoCount: event.target.value, thresholdProvenance: 'operator_entered' })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Deadline local<input aria-label="Activation objective deadline local" type="datetime-local" value={objectiveDraft.deadlineLocal} onChange={event => updateObjective({ deadlineLocal: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" />{objectiveDraft.deadlineLocal && <span className="block mt-1 normal-case text-slate-500">Local ToughBook time{objectiveUtc ? `; UTC ${objectiveUtc}` : '; UTC derived when validated'}</span>}</label></div><p className="text-[10px] text-slate-400">{objectiveDraft.thresholdProvenance === 'program_default' ? 'Program default threshold; editing it records operator-entered provenance.' : 'Threshold entered by operator.'}</p>{objectiveError && <p role="alert" className="text-[10px] text-red-200">{objectiveError}</p>}<button type="button" disabled={starting} onClick={start} className="min-h-11 rounded border border-emerald-600 px-4 py-2 text-[11px] font-black text-emerald-200 disabled:opacity-50">{starting ? 'STARTING ACTIVATION...' : 'START ACTIVATION'}</button></section>}
