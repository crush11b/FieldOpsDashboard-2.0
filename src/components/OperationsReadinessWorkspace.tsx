...
import type { Activation, ActivationObjectiveSelection, ActivationOperatingObjective } from '../../server/activation';
...
import { formatWeatherHour } from '../timePresentation';
...
export const OperationsReadinessWorkspace: React.FC<OperationsReadinessWorkspaceProps> = ({ brief, initialActivation = null, onStartActivation, onSaveObjective }) => {
  ...
  const [clockAttempt, setClockAttempt] = useState<ClockSynchronizationEvidence | null>(null);
...
const ReadinessContent: React.FC<{
  ...
  const [objectiveDraft, setObjectiveDraft] = useState<ActivationObjectiveDraft>(() => initialActivation?.operatingObjective ? draftFromObjective(initialActivation.operatingObjective) : defaultActivationObjective(brief.activation.program));
  const [objectiveError, setObjectiveError] = useState<string | null>(null);
  const [objectiveUtc, setObjectiveUtc] = useState<string | null>(() => initialActivation?.operatingObjective?.deadlineUtc || null);
  const [objectiveSelection, setObjectiveSelection] = useState<ObjectiveSelectionState>(() => initialObjectiveSelection(initialActivation, brief.activation.program));
  const completed = initialActivation?.status === 'completed';
  const canEditObjective = !completed && Boolean(onStartActivation || onSaveObjective);
  useEffect(() => { if (initialActivation?.operatingObjective) setObjectiveDraft(draftFromObjective(initialActivation.operatingObjective)); setObjectiveSelection(initialObjectiveSelection(initialActivation, brief.activation.program)); setObjectiveUtc(initialActivation?.operatingObjective?.deadlineUtc || null); }, [initialActivation, brief.activation.program]);
  const updateObjective = (changes: Partial<ActivationObjectiveDraft>) => { setObjectiveDraft(current => ({ ...current, ...changes })); setObjectiveSelection('operator_entered'); setObjectiveError(null); if ('deadlineLocal' in changes) setObjectiveUtc(changes.deadlineLocal ? localDateTimeToUtc(changes.deadlineLocal) || null : null); };
  const chooseDefault = () => { setObjectiveDraft(defaultActivationObjective(brief.activation.program)); setObjectiveSelection('program_default'); setObjectiveError(null); };
  const withoutObjective = () => { setObjectiveSelection('explicitly_absent'); setObjectiveError(null); setObjectiveUtc(null); };
  const save = () => { if (objectiveSelection === 'legacy_unavailable') { setObjectiveError('Choose an explicit objective decision before saving.'); return; } const selection = objectiveSelection === 'proposed_program_default' ? 'program_default' : objectiveSelection; if (selection === 'explicitly_absent') { void (onSaveObjective || onStartActivation)?.(undefined, selection); return; } const normalized = normalizeActivationObjective(objectiveDraft); if (!normalized.objective) { setObjectiveError(normalized.error || 'The objective is invalid.'); return; } setObjectiveUtc(normalized.deadlineUtc ?? null); void (onSaveObjective || onStartActivation)?.(normalized.objective, selection); };
  if (completed) return <section className="rounded-lg border border-emerald-700/70 bg-emerald-950/20 p-3 space-y-2" aria-label={`${brief.activation.program} prepare summary`}><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-[11px] uppercase text-emerald-300">{brief.activation.program} PREPARE</strong><span className="text-[10px] text-slate-400">{formatUtc(summary.evaluatedAtUtc)}</span></div><p className="text-[11px] text-slate-300">Completed Activation. This PREPARE record is historical and read-only; review retained evidence in REVIEW.</p><p className="text-[10px] text-slate-400">Completed at: {initialActivation?.endedAtUtc ? formatUtc(initialActivation.endedAtUtc) : 'Unknown historical time'}.</p></section>;
  return <>
    <section className="rounded-lg border border-emerald-700/70 bg-emerald-950/20 p-3 space-y-2" aria-label={`${brief.activation.program} prepare summary`}>
      <div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-[11px] uppercase text-emerald-300">{brief.activation.program} PREPARE</strong><span className="text-[10px] text-slate-400">{formatUtc(summary.evaluatedAtUtc)}</span></div>
      {completed && <p className="text-[11px] text-slate-300">Completed Activation. This PREPARE record is historical and read-only; review retained evidence in REVIEW.</p>}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{readinessCard('Location', `${brief.plannedOperatingSite.location.gridSquare || 'Site unavailable'} / ${findingMessage(summary, 'current-location')}`, findFinding(summary, 'current-location')?.status)}{readinessCard('GPS', findingMessage(summary, 'current-location'), findFinding(summary, 'current-location')?.status)}<ClockReadinessCard finding={findFinding(summary, 'clock-synchronization')} clockAttempt={clockAttempt} onSynchronizeClock={onSynchronizeClock} />{readinessCard('ToughBook', summary.toughBook.status === 'ready' ? `${summary.toughBook.chargePercent ?? 'Unknown'}% / ${summary.toughBook.powerSource}` : 'Power unknown', summary.toughBook.status)}{readinessCard('Weather', weatherText, displayEvidence.weather.status === 'not_requested' ? 'unknown' : findFinding(summary, 'weather')?.status)}{readinessCard('Alerts', alertsText, displayEvidence.alerts.status === 'not_requested' ? 'unknown' : findFinding(summary, 'weather-alerts')?.status)}{readinessCard('Space Weather', 'Retained plan evidence', 'ready')}{readinessCard('Propagation', findingMessage(summary, 'propagation-evidence'), findFinding(summary, 'propagation-evidence')?.status)}{readinessCard('Checklist', findingMessage(summary, 'field-readiness-checklist'), findFinding(summary, 'field-readiness-checklist')?.status, findFinding(summary, 'field-readiness-checklist')?.message.includes('has not been started') ? 'NOT STARTED' : undefined)}</div>
      {(onStartActivation || onSaveObjective) && <section className="rounded border border-emerald-800 bg-slate-950/40 p-2 space-y-2" aria-label="Activation objective"><h4 className="text-[10px] font-black uppercase text-emerald-300">ACTIVATION OBJECTIVE</h4><p className="text-[11px] text-slate-200">Choose an explicit objective for this Activation, or record that no objective is selected.</p>{objectiveSelection === 'proposed_program_default' && <p className="text-[10px] text-cyan-200">PROPOSED PROGRAM DEFAULT: starting or saving will accept and retain this proposal.</p>}<div className="flex flex-wrap gap-2">{brief.activation.program !== 'General' && <button type="button" onClick={chooseDefault} className={`min-h-10 rounded border px-3 py-2 text-[10px] font-bold ${objectiveSelection === 'program_default' ? 'border-emerald-400 text-emerald-200' : 'border-slate-700 text-slate-300'}`}>ACCEPT PROPOSED DEFAULT</button>}<button type="button" onClick={() => { setObjectiveSelection('operator_entered'); setObjectiveError(null); }} className={`min-h-10 rounded border px-3 py-2 text-[10px] font-bold ${objectiveSelection === 'operator_entered' ? 'border-emerald-400 text-emerald-200' : 'border-slate-700 text-slate-300'}`}>CUSTOMIZE</button><button type="button" onClick={withoutObjective} className={`min-h-10 rounded border px-3 py-2 text-[10px] font-bold ${objectiveSelection === 'explicitly_absent' ? 'border-amber-400 text-amber-200' : 'border-slate-700 text-slate-300'}`}>PROCEED WITHOUT OBJECTIVE</button></div>{objectiveSelection !== 'explicitly_absent' && <div className="grid grid-cols-1 gap-2 sm:grid-cols-4"><label className="text-[9px] font-bold uppercase text-slate-400">Goal<select aria-label="Activation objective goal" value={objectiveDraft.goal} onChange={event => updateObjective({ goal: event.target.value as ActivationObjectiveDraft['goal'] })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100"><option value="secure_activation">Secure activation</option><option value="maximize_contacts">Maximize contacts</option><option value="chase_dx">Chase DX</option><option value="explore_bands">Explore bands</option></select></label><label className="text-[9px] font-bold uppercase text-slate-400">Label<input aria-label="Activation objective label" value={objectiveDraft.label} onChange={event => updateObjective({ label: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Threshold (QSOs)<input aria-label="Activation objective threshold" type="number" min="1" step="1" value={objectiveDraft.requiredQsoCount} onChange={event => updateObjective({ requiredQsoCount: event.target.value, thresholdProvenance: 'operator_entered' })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" /></label><label className="text-[9px] font-bold uppercase text-slate-400">Deadline local<input aria-label="Activation objective deadline local" type="datetime-local" value={objectiveDraft.deadlineLocal} onChange={event => updateObjective({ deadlineLocal: event.target.value })} className="mt-1 min-h-10 w-full rounded border border-slate-700 bg-slate-950 px-2 text-[11px] text-slate-100" />{objectiveDraft.deadlineLocal ? <span className="block mt-1 normal-case text-slate-500">Local ToughBook time; UTC {objectiveUtc || 'derived when validated'}</span> : <span className="block mt-1 normal-case text-slate-500">No explicit deadline selected</span>}</label></div>}<p className="text-[10px] text-slate-400">Selection provenance: {objectiveSelection === 'proposed_program_default' ? 'Proposed program default.' : objectiveSelection === 'program_default' ? 'Accepted proposed program default.' : objectiveSelection === 'operator_entered' ? 'Operator entered or customized.' : objectiveSelection === 'legacy_unavailable' ? 'Objective selection provenance unavailable for legacy record.' : 'Explicitly absent.'}</p>{objectiveError && <p role="alert" className="text-[10px] text-red-200">{objectiveError}</p>}<button type="button" disabled={starting || objectiveSelection === 'legacy_unavailable'} onClick={save} className="min-h-11 rounded border border-emerald-600 px-4 py-2 text-[11px] font-black text-emerald-200 disabled:opacity-50">{starting ? 'SAVING...' : initialActivation ? 'SAVE OBJECTIVE' : 'START ACTIVATION'}</button></section>}
...
const WeatherEvidence: React.FC<{ evidence: OperationsReadinessDisplayEvidence; evaluatedAtUtc: string; loading: boolean; onLoad: () => void }> = ({ evidence, evaluatedAtUtc, loading, onLoad }) => {
  ...
  return <EvidenceSection title="DAY-OF LIVE CONDITIONS / PLANNED SITE WEATHER / ALERTS">
    ...
    {weather.status === 'live' && <p className="text-[10px] text-slate-400">Hourly forecast (UTC; target timezone not retained): {weather.data?.hourlyForecast && weather.data.hourlyForecast.length > 0 ? weather.data.hourlyForecast.slice(0, 3).map(item => `${formatWeatherHour(item.startsAtUtc, 'UTC')?.clock ?? 'Unknown time'} UTC ${item.tempF}°F, ${item.precipProb}% rain`).join(' | ') : 'Unavailable'} <span className="text-slate-500">UTC retrieval: {formatUtc(weather.retrievedAtUtc || evaluatedAtUtc)}</span></p>}
...
const readinessCard = (label: string, value: string, status?: string, statusText?: string) => <div key={label} className="rounded border border-slate-800 bg-slate-900/70 p-2"><span className="block text-[9px] uppercase text-slate-500">{label}</span><strong className="block mt-0.5 text-[10px] text-slate-200">{statusText || (status ? status.toUpperCase() : 'UNKNOWN')}</strong><span className="block mt-0.5 text-[10px] text-slate-400 break-words">{value}</span></div>;

const ClockReadinessCard: React.FC<{ finding?: ReadinessFinding; clockAttempt: ClockSynchronizationEvidence | null; onSynchronizeClock: () => void }> = ({ finding, clockAttempt, onSynchronizeClock }) => { const [confirmed, setConfirmed] = useState(false); const status = finding?.status; const canSynchronize = status === 'attention' || status === 'ready'; return <div className="rounded border border-slate-800 bg-slate-900/70 p-2"><span className="block text-[9px] uppercase text-slate-500">Clock</span><strong className="block mt-0.5 text-[10px] text-slate-200">{status ? status.toUpperCase() : 'UNKNOWN'}</strong><span className="block mt-0.5 text-[10px] text-slate-400 break-words">{finding?.message || 'Unknown'}</span>{clockAttempt && <span role="status" className="block mt-1 text-[10px] text-cyan-200">Last synchronization result: {clockAttempt.status} / {clockAttempt.error}. {clockAttempt.attemptMessage || 'No additional result message.'}</span>}{canSynchronize && <div className="mt-2 space-y-1"><label className="flex items-start gap-1 text-[9px] text-amber-100"><input aria-label="Confirm Windows clock synchronization" type="checkbox" className="mt-0.5" checked={confirmed} onChange={event => setConfirmed(event.currentTarget.checked)} /> Confirm fresh GNSS UTC may set Windows time.</label><button type="button" disabled={!confirmed} className="min-h-8 px-2 py-1 rounded border border-cyan-700 text-cyan-200 text-[9px] font-bold disabled:opacity-50" onClick={() => { onSynchronizeClock(); setConfirmed(false); }}>SYNCHRONIZE WINDOWS TIME</button></div>}</div>; };
...
function toLocalDateTimeInput(value: string): string { const date = new Date(value); if (Number.isNaN(date.getTime())) return ''; const pad = (part: number) => String(part).padStart(2, '0'); return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`; }
