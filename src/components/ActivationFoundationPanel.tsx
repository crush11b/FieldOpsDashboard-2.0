...
export const ActivationFoundationPanel: React.FC<ActivationFoundationPanelProps> = ({ brief, initialActivation = null, onActivationChange, showReview = true }) => {
  ...
  return <section className="rounded-xl border border-cyan-700/70 bg-cyan-950/20 p-3 space-y-3" aria-label="Activation">
    ...
    {activation && <><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Detail label="PLANNED LOCATION" value={location ? `${location.gridSquare || 'Grid unavailable'}` : 'Unknown'} /><Detail label="MISSION WINDOW" value={activation.missionWindow ? `${formatUtc(activation.missionWindow.start)} to ${formatUtc(activation.missionWindow.end)}` : 'Unknown'} /></div><div className="flex flex-wrap items-center gap-2">{activation.status === 'planned' && <button type="button" disabled={busy} onClick={() => void start()} className="min-h-11 rounded border border-emerald-700 px-3 py-2 text-[10px] font-bold text-emerald-200">START ACTIVATION</button>}{activation.status === 'active' && <button type="button" disabled={busy} onClick={() => void changeStatus('completed')} className="min-h-11 rounded border border-amber-700 px-3 py-2 text-[10px] font-bold text-amber-200">END ACTIVATION</button>}{showReview && <button type="button" onClick={() => setReviewOpen(open => !open)} className="min-h-11 rounded border border-emerald-700 px-3 py-2 text-[10px] font-bold text-emerald-200">{reviewOpen ? 'CLOSE REVIEW' : 'REVIEW ACTIVATION'}</button>}{activation.notesCollectionId && <a href="#activation-notes" className="min-h-11 rounded border border-cyan-700 px-3 py-3 text-[10px] font-bold text-cyan-200">OPEN ASSOCIATED NOTES</a>}</div><details className="rounded border border-slate-700 bg-slate-950/50 p-2"><summary className="cursor-pointer text-[10px] font-black uppercase text-cyan-300">Technical Details</summary><div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2"><Detail label="ACTIVATION RECORD" value={activation.activationId} /><Detail label="BRIEF" value={activation.briefId || 'Not associated'} /></div></details>{showReview && reviewOpen && <ActivationReviewPanel activation={activation} />}<QsoLoggerPanel activation={activation} /></>}
