...
describe('SmartDeploy brief rendering', () => {
  ...
  it('presents retained multi-day forecast periods compactly and discloses hourly evidence', async () => {
    ...
    render(<SmartDeployBriefView brief={v2Brief} />);
    await waitFor(() => expect(screen.getByText(/Compact operating periods · Operator local time/)).toBeTruthy());
    expect(screen.getByText(/Operating period/)).toBeTruthy();
    expect(screen.getAllByText(/partial coverage/).length).toBeGreaterThan(0);
    fireEvent.click(screen.getByText('Underlying hourly evidence / local time'));
  ...
  it('keeps retained evidence visible and marks an explicit failed refresh distinctly', async () => {
    ...
    render(<SmartDeployBriefView brief={v2Brief} />);
    await waitFor(() => expect(screen.getByText(/Hourly evidence · Operator local time/)).toBeTruthy());
  ...
  it('updates an existing active objective-less Activation through the persisted objective path', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Objective choice: Accepted proposed program default.')).toBeTruthy());
