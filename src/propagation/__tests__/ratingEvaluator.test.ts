...
describe('Slice 5H-B explainable propagation rating evaluator', () => {
  ...
  it('qualifies current disturbed, severe, and blackout environments after observations', () => {
    const disturbed = { kp: product(6), rScale: product(0) };
    const excellentUnconfirmed = assessment({ model: modelFor(80), observedRf: { state: 'live', reportCount: 0, uniquePathCount: 0, uniqueRemoteCallsignCount: 0, modeCounts: {} }, environment: disturbed });
    expect(excellentUnconfirmed.rating).toBe('GOOD');
    expect(excellentUnconfirmed.ratingDecisionSteps.map(step => step.ruleId)).toContain('environment_disturbed_qualification');
    const goodUnconfirmed = assessment({ model: modelFor(60), observedRf: { state: 'live', reportCount: 0, uniquePathCount: 0, uniqueRemoteCallsignCount: 0, modeCounts: {} }, environment: disturbed });
    expect(goodUnconfirmed.rating).toBe('FAIR');
    expect(goodUnconfirmed.ratingDecisionSteps.map(step => step.ruleId)).toContain('environment_disturbed_qualification');
    const fairUnconfirmed = assessment({ model: modelFor(35), observedRf: { state: 'unavailable', reportCount: 0, uniquePathCount: 0, uniqueRemoteCallsignCount: 0, modeCounts: {} }, environment: disturbed });
    expect(fairUnconfirmed.rating).toBe('FAIR');
    expect(fairUnconfirmed.ratingDecisionSteps.map(step => step.ruleId)).not.toContain('environment_disturbed_qualification');
    const poorUnconfirmed = assessment({ model: modelFor(0), observedRf: { state: 'unavailable', reportCount: 0, uniquePathCount: 0, uniqueRemoteCallsignCount: 0, modeCounts: {} }, environment: disturbed });
    expect(poorUnconfirmed.rating).toBe('POOR');
    expect(poorUnconfirmed.ratingDecisionSteps.map(step => step.ruleId)).not.toContain('environment_disturbed_qualification');
    const openingProtected = assessment({ model: modelFor(35), environment: disturbed });
    expect(openingProtected.rating).toBe('GOOD');
    expect(openingProtected.cautions.map(caution => caution.code)).toContain('current_conditions_disturbed');
    expect(openingProtected.ratingDecisionSteps.map(step => step.ruleId)).not.toContain('environment_disturbed_qualification');
    const poorOpeningProtected = assessment({ model: modelFor(0), environment: disturbed });
    expect(poorOpeningProtected.rating).toBe('FAIR');
    expect(poorOpeningProtected.ratingDecisionSteps.map(step => step.ruleId)).not.toContain('environment_disturbed_qualification');
    const observedOnlyStrong = assessment({ band: '6m', model: modelFor(null, 'unsupported'), environment: disturbed });
    expect(observedOnlyStrong.rating).toBe('GOOD');
    expect(observedOnlyStrong.ratingDecisionSteps.map(step => step.ruleId)).not.toContain('environment_disturbed_qualification');
    const observedOnlyModest = assessment({ band: '6m', model: modelFor(null, 'unsupported'), observedRf: { reportCount: 1, uniquePathCount: 1, uniqueRemoteCallsignCount: 1 }, environment: disturbed });
    expect(observedOnlyModest.rating).toBe('FAIR');
    expect(observedOnlyModest.ratingDecisionSteps.map(step => step.ruleId)).not.toContain('environment_disturbed_qualification');
    const staleObserved = assessment({ model: modelFor(80), observedRf: { state: 'stale' }, environment: disturbed });
    expect(staleObserved.rating).toBe('GOOD');
    expect(staleObserved.ratingDecisionSteps.map(step => step.ruleId)).toContain('environment_disturbed_qualification');
    const cachedObserved = assessment({ model: modelFor(80), observedRf: { state: 'cached' }, environment: disturbed });
    expect(cachedObserved.rating).toBe('GOOD');
    expect(cachedObserved.ratingDecisionSteps.map(step => step.ruleId)).toContain('environment_disturbed_qualification');
