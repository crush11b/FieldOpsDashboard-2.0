...
export function assembleMissionGuidance(input: MissionGuidanceInput): MissionGuidance {
  ...
  if (input.retrospective) {
    ...
    const objectiveResult = objective ? remaining === 0 ? 'Objective met' : 'Objective not met' : 'No objective selected';
    const progress = required === null ? `${qsoEvidence.total} QSOs` : `${qsoEvidence.total}/${required} QSOs`;
    return { ...common, urgency: remaining === 0 ? 'complete' as const : 'routine' as const, action: `Activation complete. ${objectiveResult}. Progress: ${progress}. At activation end, ${comparison} This is a retrospective comparison, not a command to resume operating.`, reasons: ['This assessment is bounded by the retained Activation completion time and retained evidence.', 'The assessment describes the completed activation and does not direct future operation.'], reconsiderWhen: 'Reassessment would have been triggered only during the completed activation by the retained progress or evidence boundary.' };
  }
